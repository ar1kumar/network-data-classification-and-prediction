...
def knn(training_set, test_set, k):
    ...
    try:
        #Output should be in CSV format for the ease of reading
        print('\n-----------------\n')

        #variable to track the number of predicted test instances
        instance_counter = 0
        test_set_length = len(test_set)
        print('Total number of test instances to predict - '+str(test_set_length))
        print('\nPredicting class for test dataset. Please wait..')
        #Open existing or create a new csv file to write the predicted outputs
        #Print output to a csv file
        output_csv_filename = "predicted_class_"+time.strftime("%Y-%m-%d-%H:%M")+".csv"
        with open(output_csv_filename,"w") as output_file:
            for test_instance in test_set:
                ...
                # test_instance_as_csv = ','.join(str(x) for x in test_instance)
                test_instance.append(classes[index])
                if(instance_counter < test_set_length):
                    print('\nPredicting class for test_instance ID: ' +str(instance_counter)+'/'+str(test_set_length))
                    instance_counter += 1
                # print(test_instance)

                #Write predicted values to csv file, write python list as a row
                writer = csv.writer(output_file)
                writer.writerow(test_instance)
